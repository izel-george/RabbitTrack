...
interface AddBuckModalProps {
  open: boolean
  onClose: () => void
  onSaved: () => void
}

export function AddBuckModal({ open, onClose, onSaved }: AddBuckModalProps) {
  ...
  async function handleSave() {
    if (!name.trim()) {
      setError('Give this buck a name.')
      return
    }
    setSaving(true)
    setError(null)
    try {
      await api.bucks.create({ name: name.trim(), sex: 'M', hatchDate: hatchDate || undefined })
      onSaved()
      onClose()
      setName('')
      setHatchDate('')
